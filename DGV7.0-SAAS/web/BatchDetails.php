<?php session_start();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'cancel_all') {
            $update = mysqli_query($connection_server, "UPDATE sas_bulk_queue_items SET status='done', response_desc='Cancelled by user', processed_at=NOW() WHERE vendor_id='$vendor_id' AND username='$username' AND batch_number='$batch' AND status IN ('pending', 'processing')");
            if ($update) {
                $_SESSION['batch_action_msg'] = ["status" => "success", "text" => "All pending transactions in this batch have been cancelled."];
            } else {
            ...
            if (!empty($ids)) {
                $ids_csv = implode(',', $ids);
                $update = mysqli_query($connection_server, "UPDATE sas_bulk_queue_items SET status='done', response_desc='Cancelled by user', processed_at=NOW() WHERE vendor_id='$vendor_id' AND username='$username' AND batch_number='$batch' AND id IN ($ids_csv) AND status IN ('pending', 'processing')");
                if ($update) {
                    $_SESSION['batch_action_msg'] = ["status" => "success", "text" => "Selected pending transactions have been cancelled."];
                } else {
            ...
            $check_tx = mysqli_query($connection_server, "SELECT * FROM sas_transactions WHERE vendor_id='$vendor_id' AND username='$username' AND batch_number='$batch' AND reference='$legacy_ref' AND status='2'");
            if ($check_tx && mysqli_num_rows($check_tx) == 1) {
                ...
                if ($charge === "success") {
                    mysqli_query($connection_server, "UPDATE sas_transactions SET status='0', description='Cancelled by user' WHERE reference='$legacy_ref'");
    ...
    <section class="section dashboard">
      <div class="col-12">
        <?php
            ...
            $get_user_transaction_details = mysqli_query($connection_server, "SELECT * FROM sas_transactions WHERE vendor_id='".$get_logged_user_details["vendor_id"]."' AND username='".$get_logged_user_details["username"]."' AND batch_number='$batch' ORDER BY date DESC");
            ...
            $has_pending = false;
            $get_queue_items = mysqli_query($connection_server, "SELECT * FROM sas_bulk_queue_items WHERE vendor_id='".$get_logged_user_details["vendor_id"]."' AND username='".$get_logged_user_details["username"]."' AND batch_number='$batch' ORDER BY id DESC");
            if ($get_queue_items) {
                            ...
                            if (empty($tx_references) && empty($uncharged_queue_items)) {
                                ...
                                foreach ($uncharged_queue_items as $qi_row) {
                                    ...
                                    if ($qi_row['status'] === 'done') {
                                        ...
                                        $qi_desc = '<button type="button" class="btn btn-danger btn-sm" onclick="cancelSingleItem('.$qi_row['id'].')">Cancel</button>';
                                    }
                                    echo '<tr>';
                                    if ($has_pending) {
                                        if ($is_item_pending) {
