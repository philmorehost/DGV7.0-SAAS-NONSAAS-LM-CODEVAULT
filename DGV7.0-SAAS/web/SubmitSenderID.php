...
function isTrivialMessage($msg)
{
    ...
    if (in_array($msg_clean, $banned))
        return true;
    if (str_word_count($msg_clean) < 5)
        return true;
    if (strlen($msg_clean) < 20)
        return true;
    return false;
}
...
function auto_submit_sender_id_to_api($connection_server, $vendor_id, $s_id, $s_msg) {
    ...
    // PhilmoreSMS
    $pms_api_q = mysqli_query($connection_server, "SELECT api_key, api_base_url FROM sas_apis WHERE vendor_id='$vendor_id' AND api_type='bulk-sms' AND api_base_url LIKE '%philmoresms.com%' AND status='1' LIMIT 1");
    if($pms_api_q && mysqli_num_rows($pms_api_q) > 0){
        $pms_api_row = mysqli_fetch_assoc($pms_api_q);
        $pms_token   = trim($pms_api_row['api_key']);
        $log_data .= "Found PhilmoreSMS API in DB. Base URL: " . $pms_api_row['api_base_url'] . "\n";
        ...
        $log_data .= "PhilmoreSMS not active or not found in sas_apis for this vendor.\n";
    }

    // KudiSMS
    $kudi_api_q = mysqli_query($connection_server, "SELECT api_key, api_base_url FROM sas_apis WHERE vendor_id='$vendor_id' AND api_type='bulk-sms' AND api_base_url LIKE '%kudisms.net%' AND status='1' LIMIT 1");
    if($kudi_api_q && mysqli_num_rows($kudi_api_q) > 0){
        ...
        $log_data .= "Found KudiSMS API in DB. Base URL: " . $kudi_api_row['api_base_url'] . "\n";
        ...
        $log_data .= "KudiSMS not active or not found in sas_apis for this vendor.\n";
    }
...
<body>
            ...
            <div class="col-lg-8">
                <div class="card shadow-sm border-0 p-4 mb-4">
                    ...
                    <form method="post" action="">
                        <div class="mb-3">
                            <label class="form-label small fw-bold text-uppercase text-muted">Sender ID (3-11
                                Letters)</label>
                            <input name="sender-id" type="text" placeholder="e.g PHILMORESMS" pattern="[a-zA-Z]{3,11}"
                                class="form-control form-control-lg fw-bold text-center" required />
                            <small class="text-muted" style="font-size: 10px;">Only letters allowed. No spaces or
                                special characters.</small>
                        </div>
                        <div class="mb-2">
                            <label class="form-label small fw-bold text-uppercase text-muted">Sample Message</label>
                            <textarea name="sample-message" rows="3" id="new-sample-msg"
                                placeholder="Enter a realistic sample of the message you intend to send e.g. 'Dear Customer, your order #12345 has been dispatched and will arrive within 24 hours. Visit our website for tracking.'"
                                class="form-control" minlength="20" maxlength="160" required></textarea>
                            <div class="d-flex justify-content-between mt-1">
                                <small class="text-muted" style="font-size: 10px;">Minimum 5 words &amp; 20 characters.
                                    No greetings or test messages.</small>
                                <small id="new-msg-counter" class="text-muted" style="font-size: 10px;">0/160</small>
                            </div>
                        </div>
                        <button name="submit-sender-id" type="submit"
                            class="btn btn-primary btn-lg w-100 shadow-sm fw-bold mt-3">
                            SUBMIT FOR REVIEW
                        </button>
                    </form>
                </div>
                    ...
                    <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                        ...
                        <form method="get" action="SubmitSenderID.php" class="d-flex gap-2">
                            <input name="searchq" type="text"
                                ...
                                placeholder="Search ID..." class="form-control form-control-sm rounded-pill px-3" />
                            ...
                            <tbody>
                                <?php if (mysqli_num_rows($get_user_transaction_details) > 0): ?>
                                    <?php while ($row = mysqli_fetch_assoc($get_user_transaction_details)):
                                        ...
                                        ?>
                                        <tr>
                                            <td class="px-4 fw-bold text-primary"><?php echo strtoupper($row["sender_id"]); ?>
                                            </td>
                                            <td class="small text-muted"><?php echo htmlspecialchars($row["sample_message"]); ?>
                                            </td>
                                            <td class="small <?php echo $status_class; ?> fw-bold">
                                                <?php echo tranStatus($row["status"]); ?></td>
                                            <td class="text-end px-4 small text-muted">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center py-5 text-muted">No Sender IDs found.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    ...
                    <div class="card-footer bg-white py-3 border-0 d-flex justify-content-between">
                        <div>
                            <?php if (isset($_GET["page"]) && $_GET["page"] > 1): ?>
                                <a href="SubmitSenderID.php?<?php echo $search_parameter; ?>page=<?php echo ($_GET["page"] - 1); ?>"
                                    class="btn btn-outline-primary btn-sm rounded-pill px-3">Previous</a>
                            <?php endif; ?>
                        </div>
                        <?php
                        $next_page = ($_GET["page"] ?? 1) + 1;
                        ?>
                        <a href="SubmitSenderID.php?<?php echo $search_parameter; ?>page=<?php echo $next_page; ?>"
                            class="btn btn-outline-primary btn-sm rounded-pill px-3">Next</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="editSenderIDModal" tabindex="-1" aria-labelledby="editSenderIDModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-4 shadow border-0">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-bold" id="editSenderIDModalLabel"><i
                            class="bi bi-pencil-square me-2 text-warning"></i>Edit Sender ID</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form method="post" action="">
                    <div class="modal-body p-4">
                        <div class="alert alert-warning border-0 small rounded-3 mb-3">
                            <i class="bi bi-info-circle me-1"></i>Editing will <strong>reset the status to
                                Pending</strong> and re-submit for admin review.
                        </div>
                        <div class="mb-3">
                            <label class="form-label small fw-bold text-uppercase text-muted">Sender ID</label>
                            <input id="edit-sender-id-display" type="text"
                                class="form-control fw-bold text-center bg-light" readonly />
                            <input id="edit-sender-id-value" name="edit-sender-id-value" type="hidden" />
                        </div>
                        <div class="mb-2">
                            <label class="form-label small fw-bold text-uppercase text-muted">Sample Message</label>
                            <textarea id="edit-sample-message" name="edit-sample-message" rows="4" class="form-control"
                                minlength="20" maxlength="160"
                                placeholder="Enter a realistic sample message (minimum 5 words, 20 characters)"
                                required></textarea>
                            <div class="d-flex justify-content-between mt-1">
                                <small class="text-muted" style="font-size: 10px;">Minimum 5 words &amp; 20
                                    characters.</small>
                    ...
                    <div class="modal-footer border-0 pt-0">
                        ...
                        <button name="edit-sender-id" type="submit"
                            class="btn btn-warning rounded-pill px-4 fw-bold">Save &amp; Resubmit</button>
                    </div>
                </form>
            </div>
        </div>
