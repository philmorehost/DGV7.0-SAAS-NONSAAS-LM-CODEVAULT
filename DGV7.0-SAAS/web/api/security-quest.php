...
if (empty($api_key) || empty($action)) {
    echo json_encode(["status" => "failed", "desc" => "Missing required parameters: api_key or action."]);
    exit;
}
...
switch ($action) {
    ...
    case "set":
        if ($has_quest) {
            echo json_encode(["status" => "failed", "desc" => "Security question already set. Use action=reset instead."]);
            break;
        }
        ...
        if (empty($quest_id) || !is_numeric($quest_id) || empty($answer)) {
            echo json_encode(["status" => "failed", "desc" => "Security answer cannot be empty."]);
            break;
        }
        if (strlen($answer) < 3 || strlen($answer) > 20) {
            echo json_encode(["status" => "failed", "desc" => "Security answer must be between 3-20 characters."]);
            break;
        }
        ...
        echo json_encode(["status" => "success", "desc" => "Security question set successfully."]);
        break;
    ...
    case "verify":
        ...
        if ($has_quest && $answer === $user["security_answer"]) {
            echo json_encode(["status" => "success", "desc" => "Security verification successful."]);
        } else {
            echo json_encode(["status" => "failed", "desc" => "Invalid security answer."]);
        }
        break;
    ...
    case "reset":
        ...
        if (mysqli_num_rows($q_check) != 1) {
            echo json_encode(["status" => "failed", "desc" => "Incorrect password."]);
            break;
        }
        // alterUser() escapes internally — pass raw trimmed values, not pre-escaped ones.
        alterUser($user["username"], "security_quest", $quest_id);
        alterUser($user["username"], "security_answer", $answer);
        echo json_encode(["status" => "success", "desc" => "Security details reset successfully."]);
        break;
