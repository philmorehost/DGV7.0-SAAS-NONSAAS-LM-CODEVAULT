<?php session_start();
    ...
    if(isset($_POST["buy-bundle-card"])){
        ...
        $business_name = mysqli_real_escape_string($connection_server, trim(strip_tags($_POST["business_name"] ?? '')));
        $display_price = mysqli_real_escape_string($connection_server, trim(strip_tags($_POST["display_price"] ?? '')));
            ...
            if(userBalance(1) >= $total_price){
                ...
                if($debit === "success"){
                    ...
                    for($i = 0; $i < $quantity; $i++){
                        ...
                        mysqli_query($connection_server, "INSERT INTO sas_databundle_cards (vendor_id, user_id, product_id, service_type, data_type, network, plan_name, validity, business_name, price, display_price, epin, serial_number, sms_number, status, batch_reference, date_sold) VALUES ('".$get_logged_user_details["vendor_id"]."', '".$get_logged_user_details["id"]."', '".$get_plan['product_id']."', '".$get_plan['service_type']."', '".$get_plan['data_type']."', '".$get_plan['product_name']."', '".$get_plan['plan_code']."', '".$get_plan['validity_days']."', '$business_name', '$unit_price', '".(!empty($display_price) ? $display_price : 0)."', '$epin', '$sn', '$sms_number', 'Sold', '$batch_reference', CURRENT_TIMESTAMP)");
                    }
            ...
            <form method="post" action="">
                    ...
                    <select name="plan_id" id="data-plan" class="form-select form-control-lg" required>
                        ...
                        <?php
                            ...
                            if ($bundle_plans_query) {
                                ...
                                error_log('[DGV-DB-ERROR] DataBundleCard bundle_plans_query failed: ' . mysqli_error($connection_server));
                            }
                ...
                <div class="row mb-3">
                    <div class="col-md-6 mb-3 mb-md-0">
                        <label class="form-label small fw-bold text-uppercase">Brand Name (Optional)</label>
                        <input type="text" name="business_name" class="form-control form-control-lg" placeholder="Your Business Name">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-bold text-uppercase">Selling Price (Optional)</label>
                        <input type="number" name="display_price" class="form-control form-control-lg" placeholder="Price to show on card">
                    </div>
                </div>
