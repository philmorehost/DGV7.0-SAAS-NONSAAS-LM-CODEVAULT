...
if (in_array($product_name, array_keys($data_service_provider_alter_code))) {
    ...
    if (in_array($quantity, array_keys($web_data_size_array))) {
        ...
        // HDK debits the wallet on ITS side the moment it accepts the request, so ONLY its own
        // status word may fail the transaction. An unreadable/absent reply is NOT a failure: the
        // transaction stays PENDING and the requery queue establishes the truth (func/bc-gateway.php).
