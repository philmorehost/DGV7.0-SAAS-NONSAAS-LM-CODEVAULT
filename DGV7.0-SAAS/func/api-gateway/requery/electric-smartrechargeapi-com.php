<?php
    if(!empty($requery_reference)){
        ...
        if(in_array($curl_json_result["error_code"],array(1986))){
            ...
            $token = !empty($curl_json_result["token"]) ? $curl_json_result["token"] : (!empty($curl_json_result["data"]["token"]) ? $curl_json_result["data"]["token"] : "");
            $meter_no = !empty($curl_json_result["meter_number"]) ? $curl_json_result["meter_number"] : getTransaction($requery_reference, "product_unique_id");
