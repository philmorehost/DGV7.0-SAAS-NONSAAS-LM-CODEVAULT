...
function processUSSDSession($session_id, $session_msisdn, $session_operation, $session_msg, $session_from, $vendor_id) {
    ...
    // Fetch vendor settings if vendor_id is provided
    $vendor = null;
    if ($vendor_id > 0) {
    ...
    if ($session_operation == 'end' || $session_operation == 'close') {
        mysqli_query($connection_server, "DELETE FROM sas_ussd_sessions WHERE session_id='$session_id'");
        return [
    ...
    $get_session = mysqli_query($connection_server, "SELECT * FROM sas_ussd_sessions WHERE session_id='$session_id' LIMIT 1");
    ...
    if ($vendor_id <= 0 && !in_array($current_step, ['start', 'enter_epin'])) {
        return [
            "session_operation" => "end",
            "session_type" => 4,
            "session_id" => $session_id,
            "session_msg" => "Error: USSD session is invalid or expired."
        ];
    }
...
function handleEPINInput($epin_msg, $session_id, $session_msisdn, $vendor_id, $vendor) {
    ...
    if ($vendor_id <= 0) {
        // Shared USSD: Look up card first to identify the vendor
        $get_card_temp = mysqli_query($connection_server, "SELECT vendor_id FROM sas_databundle_cards WHERE epin='$epin_esc' LIMIT 1");
        if ($get_card_temp && $c_row = mysqli_fetch_assoc($get_card_temp)) {
            $vendor_id = (int)$c_row['vendor_id'];
            // Update session's vendor_id
            $session_id_esc = mysqli_real_escape_string($connection_server, $session_id);
            mysqli_query($connection_server, "UPDATE sas_ussd_sessions SET vendor_id='$vendor_id' WHERE session_id='$session_id_esc'");
            // Load vendor info
            $get_v = mysqli_query($connection_server, "SELECT * FROM sas_vendors WHERE id='$vendor_id' LIMIT 1");
            $vendor = mysqli_fetch_assoc($get_v);
        } else {
            mysqli_query($connection_server, "DELETE FROM sas_ussd_sessions WHERE session_id='$session_id'");
            return [
                "session_operation" => "end",
    ...
    // Check if vendor has enough wallet balance for per-call charge
    $per_call_charge = (float)getSuperAdminOption('ussd_per_call_charge', '0.00');
    if ($per_call_charge > 0) {
        if (!$vendor || $vendor['balance'] < $per_call_charge) {
            mysqli_query($connection_server, "DELETE FROM sas_ussd_sessions WHERE session_id='$session_id'");
            return [
                "session_operation" => "end",
                "session_type" => 4,
                "session_id" => $session_id,
                "session_msg" => "Error: Insufficient vendor balance for USSD service."
            ];
        }
    }
    ...
    mysqli_query($connection_server, "UPDATE sas_ussd_sessions SET session_data='$session_data' WHERE session_id='$session_id'");
    ...
    if (in_array($service_type, ['cable', 'electric', 'betting'])) {
        mysqli_query($connection_server, "UPDATE sas_ussd_sessions SET current_step='enter_extra_id' WHERE session_id='$session_id'");
        ...
        // Airtime, Data, Exam
        mysqli_query($connection_server, "UPDATE sas_ussd_sessions SET current_step='select_recharge_option' WHERE session_id='$session_id'");
        return [
            "session_operation" => "continue",
            "session_type" => 1,
            "session_id" => $session_id,
...
function triggerFulfillment($epin, $recipient, $extra_id, $session_id, $vendor_id, $vendor) {
    ...
    if ($result['status'] == 'success') {
        ...
        $per_call_charge = (float)getSuperAdminOption('ussd_per_call_charge', '0.00');
        if ($per_call_charge > 0 && $vendor) {
            $ref = "USSD-" . substr(str_shuffle("12345678901234567890"), 0, 15);
            $desc = "USSD Redemption Call Charge for EPIN: " . $epin;
            chargeOtherVendor($vendor['email'], 'debit', $epin, 'USSD Call Fee', $ref, $per_call_charge, $per_call_charge, $desc, $_SERVER['HTTP_HOST'] ?? 'USSD', 1);
        }
