...
<script>
    ...
    document.addEventListener('click', function(e) {
        ...
        const lockKeywords = ['buy', 'send', 'transfer', 'process', 'submit', 'pay', 'confirm', 'withdraw', 'verify', 'complete', 'fund'];
        const isActionBtn = lockKeywords.some(kw => text.includes(kw));
        ...
        if (isActionBtn && !btn.hasAttribute('data-no-lock')) {
            ...
            setTimeout(() => {
                // Ensure the button's name/value are preserved in the POST data
                // This is crucial for both 'submit' and 'button' types since disabled buttons are not sent.
                if (form && btn.name) {
                ...
                if (form && (btn.type === 'button' || btn.getAttribute('type') === 'button')) {
                    form.submit();
                }
            }, 10);
        }
