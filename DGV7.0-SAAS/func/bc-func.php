...
function resolveVendorID($force_recompute = false) {
    ...
    if (!empty($host) && isset($_SESSION[$host_key]) && $_SESSION[$host_key] > 0) {
...
function isServiceEnabled($name, $vid = null) {
    ...
    static $static_global_cache = null;
    ...
    // 1. Fetch Global Settings (Acts as Default State)
    if ($static_global_cache === null) {
        $static_global_cache = [];
        ...
        if ($gc_q) {
            while ($gc_r = mysqli_fetch_assoc($gc_q)) {
                $static_global_cache[$gc_r['service_name']] = (int)$gc_r['status'];
            }
        }
    }

    // 2. Fetch Local Vendor Settings (Acts as Personal Override/Opt-in)
    ...
    // A. If vendor HAS a local setting, it always wins (Vendor Opt-in/Override)
    if (isset($static_service_cache[$vid][$name])) {
        return (bool)$static_service_cache[$vid][$name];
    }

    // B. If vendor has NO local setting, follow the Global Super Admin default
    if (isset($static_global_cache[$name])) {
        return (bool)$static_global_cache[$name];
    }

    // C. Default fallback for services not yet in database
    $default_off = ['virtual_bank_display'];
    return !in_array($name, $default_off);
}
...
function chargeUser($type, $product_unique_id, $type_alternative, $reference, $api_reference, $amount, $discounted_amount, $description, $mode, $api_website, $status)
{
	...
	$get_vendor_det = mysqli_fetch_array(mysqli_query($connection_server, "SELECT * FROM sas_vendors WHERE id='" . $v_id . "' LIMIT 1"));
...
function getGatewayDetails($gateway, $vid = null) {
    ...
    // save). The old query used a bare LIMIT 1, so MySQL was free to hand back the blank row and the
    // provider then answered 401 "Invalid Secret Key" even though a correctly configured row existed.
    // Prefer, in order: an exact gateway_name match, then a row that actually carries a secret key,
    // then the oldest row (the original configuration).
    if ($vid > 0) {
    ...
    $sq = mysqli_query($connection_server, "SELECT * FROM sas_super_admin_payment_gateways WHERE (LOWER(TRIM(gateway_name)) = '$gateway_search' OR gateway_name LIKE '%$gateway_search%') ORDER BY (LOWER(TRIM(gateway_name)) = '$gateway_search') DESC, (secret_key IS NULL OR secret_key = '') ASC, date ASC LIMIT 1");
        ...
        if ($stmt) {
            ...
            if ($res && $row = mysqli_fetch_assoc($res)) {
                if ($row['status'] == 'Blacklisted') {
                    mysqli_stmt_close($stmt);
                    return "Blocked by Country Policy";
                }
            }
            mysqli_stmt_close($stmt);
        }
...
function recordLoginAttempt($username, $ip, $success, $vendor_id) {
    ...
    mysqli_stmt_execute($stmt);
    $res = mysqli_stmt_get_result($stmt);
    $ip_failures = ($res && $row = mysqli_fetch_assoc($res)) ? $row['failures'] : 0;
    mysqli_stmt_close($stmt);
    ...
    if (!empty($username)) {
        ...
        if ($user_found) {
            ...
            mysqli_stmt_execute($stmt);
            $res = mysqli_stmt_get_result($stmt);
            $acc_failures = ($res && $row = mysqli_fetch_assoc($res)) ? $row['failures'] : 0;
            mysqli_stmt_close($stmt);
...
function getSuperAdminOption($name, $default = '') {
    ...
    if (!$connection_server || !($connection_server instanceof mysqli)) {
        ...
        try {
            if (!@mysqli_ping($connection_server)) {
                $is_closed = true;
            }
        } catch (\Throwable $e) {
            $is_closed = true;
