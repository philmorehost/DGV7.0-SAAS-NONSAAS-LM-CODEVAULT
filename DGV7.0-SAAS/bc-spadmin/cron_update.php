<?php
/**
 * CLI Automated Update Cronjob with Email Notifications (SAAS Version)
...
// Get admin email to notify (SAAS version resolves super admin email)
$admin_email = '';
$q_admin = mysqli_query($connection_server, "SELECT email FROM sas_super_admin LIMIT 1");
...
write_log("Starting SAAS OTA auto-updater CLI job.");
...
// Get vendor primary domain if available (Vendor ID 1 is usually first vendor in SAAS)
...
try {
    ...
    // Save version in database settings (SAAS version updates sas_super_admin_options)
    ...
    write_log("Update successfully completed! SAAS platform is now running v{$new_version}.");

    // Send Success Email Notification (SAAS version notifies super admin)
    $email_subject = "SAAS Platform Automated Update Successful - Version v{$new_version}";
    $email_body = "<h3>SAAS Update Execution Complete</h3>
    <p>Your SAAS platform has been automatically updated successfully by the scheduled cron engine.</p>
    ...
    <hr>
    <p><i>DataGifting SAAS Update Agent.</i></p>";

    sendSuperAdminEmail($admin_email, $email_subject, $email_body);
    ...
    $email_subject = "SAAS Platform Auto-Update Failed - Rollback Initiated";
    if ($rollback_status === true) {
        write_log("Rollback completed successfully. Pre-update state restored.");
        $email_body = "<h3>SAAS Auto-Update Failed & Rollback Executed</h3>
        <p>The SAAS system update process encountered a fatal error and has been automatically rolled back to ensure stability.</p>
        ...
        $email_body = "<h3>CRITICAL WARNING: SAAS Auto-Update Failed & Rollback Failed</h3>
        <p>The SAAS system update process failed, and the automated system restore (rollback) also failed.</p>
        <p><b>Error Details:</b> {$error_msg}</p>
