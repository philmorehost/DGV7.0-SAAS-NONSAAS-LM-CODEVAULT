 ...
 *
 * SAAS is multi-tenant: resolveVendorID() (used deep inside chargeUser())
 * resolves via $GLOBALS['vendor_id'] when explicitly set — we set that per
 * queue item before processing so charges hit the right vendor.
 *
...
// Ensure the batch_number column exists in sas_transactions (in case the user hasn't triggered bc-tables.php)
$check_col = mysqli_query($connection_server, "SHOW COLUMNS FROM `sas_transactions` LIKE 'batch_number'");
...
function bc_process_bulk_queue_item($connection_server, array $item)
{
    ...
    // Multi-tenant: pin vendor context for this item so resolveVendorID() inside
    // chargeUser()/productIDBlockChecker()/etc. resolves the correct vendor.
    $GLOBALS['vendor_id'] = (int)$item['vendor_id'];
...
function bc_finalize_completed_bulk_batches($connection_server)
{
    ...
    while ($batch = mysqli_fetch_assoc($batches)) {
        ...
        // Total submitted numbers for this batch — sas_bulk_queue_items is the
        // authoritative record (one row per number, written at enqueue time),
        // unlike sas_transactions which only gets a row once chargeUser() actually
        // runs. Many rejection paths (insufficient balance, incomplete params,
        // blocked/locked product, gateway errors, abuse limit, suspended account)
        // never reach chargeUser() and so never produce a sas_transactions row —
        // counting failures from sas_transactions alone silently drops those.
        $total_row = mysqli_fetch_assoc(mysqli_query($connection_server, "SELECT COUNT(*) as c FROM sas_bulk_queue_items WHERE batch_number='" . mysqli_real_escape_string($connection_server, $batch['batch_number']) . "'"));
        $total_count = (int)($total_row['c'] ?? 0);

        $counts_q = mysqli_query($connection_server, "SELECT
            SUM(CASE WHEN status=1 THEN 1 ELSE 0 END) as success_count
            FROM sas_transactions WHERE batch_number='" . mysqli_real_escape_string($connection_server, $batch['batch_number']) . "'");
        if (!$counts_q) {
            error_log("BULK ERROR: Failed to get counts - " . mysqli_error($connection_server));
        }
...
function bc_generate_bulk_failure_diagnosis($connection_server, $batch, $failed_count)
{
    ...
    $GLOBALS['vendor_id'] = (int)$batch['vendor_id'];
    resolveVendorID(true);
