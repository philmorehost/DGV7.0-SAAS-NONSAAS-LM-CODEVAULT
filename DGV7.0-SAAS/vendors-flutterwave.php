<?php session_start();
	...
	$flutterwave_keys = mysqli_fetch_assoc(mysqli_query($connection_server,"SELECT * FROM sas_super_admin_payment_gateways WHERE gateway_name='flutterwave'"));
			...
			if(($flutterwave_verify_transaction["status"] == "success") && ($catch_incoming_request["data"]["status"] == "successful")){
				...
				if(!$vtx || $vtx['status'] != 1){
                    if ($vtx && $vtx['product_unique_id'] == 'plisio_activation') {
					    ...
					    chargeVendor("credit", $_SESSION["admin_session"], "Wallet Credit", $transaction_id, $amount_paid, $amount_deposited, "Flutterwave Wallet Credit - ".str_replace("_"," ",$payment_method), $_SERVER["HTTP_HOST"], "1");
                    }
