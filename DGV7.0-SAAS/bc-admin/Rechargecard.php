<?php session_start();
    ...
    if(isset($_POST["clear-all-plans"])){
        ...
        $api_q = mysqli_query($connection_server, "SELECT id FROM sas_apis WHERE vendor_id='$vid' AND api_type='rechargecard'");
        $api_ids = [];
        ...
        if(!empty($api_ids)){
            ...
            $_SESSION["product_purchase_response"] = "All Recharge Card plans cleared successfully.";
        } else {
            $_SESSION["product_purchase_response"] = "No Recharge Card API configured to clear plans for.";
        }
        header("Location: Rechargecard.php");
        exit();
    }
        ...
        <div>
          ...
          <nav>
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Recharge Card</li>
            </ol>
          </nav>
        </div>
    ...
    <section class="section dashboard">
      <div class="row g-4">
        ...
        <div class="col-lg-4">
          ...
          <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
            ...
            <div class="card-body p-4 text-center pt-0">
              <div class="mb-4">
                <div class="alert alert-info small border-0 py-2 mb-3">Select carriers to install Recharge PIN plans automatically.</div>
          ...
          <div class="card shadow-sm border-0 rounded-4 mb-4 border-start border-danger border-4 overflow-hidden">
            ...
            <div class="card-body p-4 text-center pt-2">
                <p class="small text-muted mb-3">Wipe all Recharge Card plans from the pricing table.</p>
                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL Recharge Card pricing plans? This cannot be undone.');">
        ...
        <div class="col-lg-8">
                            ...
                            if(mysqli_num_rows($select_item_lists) >= 1){
                                while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                    ...
                                    <tr>
                                        ...
                                        <td>'.$status_badge.'</td>
                                        <td class="pe-4 text-end">'.render_action_buttons($list_details["product_name"], "rechargecard", $list_details["status"]).'</td>
                                    </tr>';
                                }
                            }
                        ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-primary fs-6"><i class="bi bi-tag-fill me-2"></i>Recharge PIN Pricing (₦)</h6>
                ...
                <div class="card-body p-4">
                    ...
                    <form method="post" action="">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light small">
                            <tr>
                                <th class="ps-3">Package Name</th><th>Smart (₦)</th><th>Agent (₦)</th><th>API (₦)</th><th>Status</th><th class="text-end pe-3">Actions</th>
                            </tr>
                            </thead>
                            <tbody>
                                <?php
                                        ...
                                        if(mysqli_num_rows($product_smart_table) > 0){
                                            while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input id="'.strtolower($products).'_rechargecard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_smart_level" name="smart-price[]" type="number" step="0.01" value="'.$product_smart_details["val_2"].'" class="form-control form-control-sm text-center shadow-none border-0 bg-light" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_rechargecard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_agent_level" name="agent-price[]" type="number" step="0.01" value="'.$product_agent_details["val_2"].'" class="form-control form-control-sm text-center shadow-none border-0 bg-light" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_rechargecard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" name="api-price[]" type="number" step="0.01" value="'.$product_api_details["val_2"].'" class="form-control form-control-sm text-center shadow-none border-0 bg-light" style="max-width:90px"></td>
                                                    <td>'.$status_badge.'</td>
                                                    <td class="text-end pe-3">
                                                        <div class="btn-group btn-group-sm">
                                                            '.(($product_smart_details['status'] == 1) ?
                                                            '<a href="Rechargecard.php?action=disable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-secondary" title="Disable"><i class="bi bi-pause-fill"></i></a>' :
                                                            '<a href="Rechargecard.php?action=enable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-success" title="Enable"><i class="bi bi-play-fill"></i></a>'
                                                            ).'
                                                            <a href="Rechargecard.php?action=delete&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-danger" onclick="return confirm(\'Delete this package?\')" title="Delete"><i class="bi bi-trash"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>';
                                                $csv_price_level_array[] = strtolower($products).'_rechargecard_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).",".$product_smart_details["val_2"].",".$product_agent_details["val_2"].",".$product_api_details["val_2"];
                                            }
                                        }
                                    }
                                ?>
                        ...
                        <div class="mt-4 pt-4 border-top text-end">
                            <button name="update-price" type="submit" class="btn btn-primary rounded-pill px-5 fw-bold shadow-sm">Update All Prices</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4 overflow-hidden">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="javascript:void(0)" onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "rechargecard.csv");' class="btn btn-sm btn-light border rounded-pill px-3"><i class="bi bi-download me-1"></i>CSV Template</a>
                </div>
                <div class="card-body p-4">
                    <form method="post" enctype="multipart/form-data">
                        <div class="input-group">
                            <input id="csv-chooser" type="file" class="form-control rounded-start-3" required/>
                            <button data-no-lock onclick="getCSVDetails('4');" type="button" class="btn btn-primary px-4 fw-bold shadow-sm">Process Upload</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4 overflow-hidden">
                <div class="card-header bg-white py-3 border-bottom">
                    <h6 class="fw-bold mb-0 text-primary fs-6"><i class="bi bi-cloud-upload me-2"></i>Upload Recharge PINs (Manual)</h6>
                </div>
                <div class="card-body p-4 bg-light bg-opacity-10">
                    <form method="post" action="">
                        <div class="row g-3 mb-3">
                                    ...
                                    <?php
                                        ...
                                        if(mysqli_num_rows($get_rechargecard_products) >= 1){
                                            ...
                                            while($product_details = mysqli_fetch_assoc($get_rechargecard_products)){
                                                ...
                                                echo '<option value="'.$product_details["product_name"].'">'.strtoupper($product_details["product_name"]).$p_status.'</option>';
                                            }
                                        }
                                    ?>
                                </select>
                            ...
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">Denomination</label>
                                <select id="admin-cards-qty" name="qty" onchange="adminCardsSwitch();" class="form-select" required>
                                    <option value="" hidden selected>Choose Qty</option>
                                    <?php echo rechargecardQty(); ?>
                                </select>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label small fw-bold">Dial Code Configuration</label>
                            <input id="admin-cards-input" name="dial-code" type="text" placeholder="e.g. *311*PIN#, *310#" class="form-control" required/>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="admin-cards-textarea" name="cards" placeholder="Paste PINs here, separated by commas" class="form-control" rows="8" style="font-family: monospace;"></textarea>
                        </div>

                        <?php echo rechargecardTextarea(); ?>

                        <button name="upload-product" type="submit" class="btn btn-primary w-100 rounded-3 fw-bold py-2 shadow-sm">
                            <i class="bi bi-cloud-upload me-2"></i>Upload PINs
                        </button>
                    </form>
                </div>
            </div>
