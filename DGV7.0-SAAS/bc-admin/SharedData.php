...
if (isset($_POST["clear-all-plans"])) {
    ...
    $api_q = mysqli_query($connection_server, "SELECT id FROM sas_apis WHERE vendor_id='$vid' AND api_type='shared-data'");
    $api_ids = [];
    while ($r = mysqli_fetch_assoc($api_q))
        $api_ids[] = $r['id'];
    ...
    if (!empty($api_ids)) {
        ...
        $_SESSION["product_purchase_response"] = "All Shared Data plans cleared successfully.";
    } else {
        ...
        $_SESSION["product_purchase_response"] = "Shared Data plans cleared from pricing table.";
    }
    header("Location: SharedData.php");
    exit();
}
...
if (isset($_POST["install-product"])) {
    ...
    // NOTE: these values become `val_1`, which the api-gateway files look up verbatim
    // (e.g. func/api-gateway/shared-data-hdkdata-com.php's $web_data_size_array) — they
    // must be plain size labels with no "_Nday(s)" suffix, or every purchase fails at
    // the gateway's availability check before the API is ever called. Validity days are
    // already tracked separately in val_3 ("Days" column in the pricing table below).
    // Airtel has no supported hdkdata shared-data plan codes at all, so it's left empty.
    $product_varieties = array(
        "mtn" => array("1gb", "2.5gb", "5gb", "10gb"),
        "airtel" => array(),
                ...
                <div class="col-12">
                    <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                        <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0"><i class="bi bi-cloud-arrow-down me-2 text-primary"></i>Shared Data
                                Package Plan Variation Fetcher</h5>
                            <button class="btn btn-sm btn-outline-info rounded-pill px-3" type="button"
                                data-bs-toggle="collapse" data-bs-target="#fetcherGuide">
                        ...
                        <div class="card-body p-4">
                            <div class="collapse mb-3" id="fetcherGuide">
                                <div class="alert alert-info border-0 shadow-sm rounded-4 mb-0">
                                    ...
                                    <ol class="small mb-0 text-dark">
                                        ...
                                        <li>Enter your desired <strong>Markup</strong> percentages (e.g., set Smart % to 5
                                            to add a 5% markup).</li>
                                        <li>Click <strong>Fetch Plans</strong> to load the latest available packages from
                                            the provider.</li>
                                        <li>Click <strong>Apply</strong> on a single plan to preview it in the pricing table
                                            below, or click <strong>Apply All</strong> to save all fetched plans directly to
                                            your database.</li>
                                        <li><strong>Important:</strong> If you use "Apply" for individual plans, remember to
                                            click <strong>Save All Changes</strong> at the bottom of the pricing table to
                                            permanently save them.</li>
                                    </ol>
                                </div>
                            </div>
                            <div class="row g-3">
                                ...
                                <div class="col-md-2 d-flex align-items-end">
                                    <button type="button" onclick="fetchVariations();"
                                        class="btn btn-primary w-100 fw-bold">Fetch Plans</button>
                                </div>
                            </div>
                            ...
                            <div id="fetch-results" class="mt-4" style="display:none;">
                                ...
                                <div class="mt-3 text-end">
                                    <button type="button" onclick="applyAllFetched();" class="btn btn-success fw-bold">Apply
                                        All to Pricing Table</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    ...
                    <div class="card-body p-4 text-center">
                        <p class="small text-muted mb-4">Wipe all Shared Data plans from the pricing table. Use this
                            when switching API providers.</p>
                        <form method="post" action=""
                            onsubmit="return confirm('Are you sure you want to clear ALL Shared Data pricing plans? This cannot be undone.');">
                            <button name="clear-all-plans" type="submit"
                                class="btn btn-outline-danger w-100 rounded-3 fw-bold">
                                <i class="bi bi-eraser-fill me-2"></i>Clear All Shared Plans
                            </button>
                        </form>
                    </div>
                </div>
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div class="card-header bg-white py-3 border-0">
                        <h5 class="card-title mb-0"><i class="bi bi-gear-wide-connected me-2 text-primary"></i>API
                            Setting</h5>
                    </div>
                    <div class="card-body p-4">
                        <form method="post" action="">
                            <div class="mb-3">
                                ...
                                <select id="" name="api-id" onchange="getWebApikey(this);" class="form-select rounded-3"
                                    required>
                                    <?php
                            ...
                            <div class="mb-4">
                                ...
                                <input id="web-apikey-input" name="api-key" type="text" placeholder="Paste API Key here"
                                    class="form-control rounded-3" required />
                            </div>
                            <button name="update-key" type="submit"
                ...
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-header bg-white py-3 border-0">
                        <h5 class="card-title mb-0"><i class="bi bi-box-seam me-2 text-primary"></i>Product Installation
                        </h5>
                    </div>
                    <div class="card-body p-4 text-center">
                        <div class="mb-4">
                            <button type="button" class="btn btn-info btn-sm w-100 mb-3 rounded-pill text-white fw-bold"
                                onclick="tickProduct(this, 'all', 'api-product-name', 'install-product', 'png');">Select
                                All Carriers</button>
                            <div class="d-flex flex-wrap justify-content-center gap-2">
                                <img alt="Airtel" id="airtel-lg" src="/asset/airtel.png"
                                    onclick="tickProduct(this, 'airtel', 'api-product-name', 'install-product', 'png');"
                                    class="rounded-3 border p-1 cursor-pointer"
                                    style="width: 60px; height: 60px; object-fit: contain;" />
                                <img alt="MTN" id="mtn-lg" src="/asset/mtn.png"
                                    onclick="tickProduct(this, 'mtn', 'api-product-name', 'install-product', 'png');"
                                    class="rounded-3 border p-1 cursor-pointer"
                                    style="width: 60px; height: 60px; object-fit: contain;" />
                                <img alt="Glo" id="glo-lg" src="/asset/glo.png"
                                    onclick="tickProduct(this, 'glo', 'api-product-name', 'install-product', 'png');"
                                    class="rounded-3 border p-1 cursor-pointer"
                                    style="width: 60px; height: 60px; object-fit: contain;" />
                                <img alt="9mobile" id="9mobile-lg" src="/asset/9mobile.png"
                                    onclick="tickProduct(this, '9mobile', 'api-product-name', 'install-product', 'png');"
                                    class="rounded-3 border p-1 cursor-pointer"
                                    style="width: 60px; height: 60px; object-fit: contain;" />
                            </div>
                        </div>
                        <form method="post" action="">
                            ...
                            <div class="mb-3">
                                <label class="form-label small fw-bold text-muted text-uppercase">Assign to
                                    Gateway</label>
                                <select name="api-id" class="form-select rounded-3" required>
                                    ...
                                    if (mysqli_num_rows($get_api_lists) >= 1) {
                                        echo '<option value="" default hidden selected>Choose API</option>';
                                        while ($api_details = mysqli_fetch_assoc($get_api_lists))
                                            echo '<option value="' . $api_details["id"] . '">' . strtoupper($api_details["api_base_url"]) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                            ...
                            <button id="install-product" name="install-product" type="submit"
                                class="btn btn-outline-primary w-100 rounded-3 fw-bold border-2"
            ...
            <div class="col-lg-8">
                            ...
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>' . $status_badge . '</td>
                                        <td class="pe-4 text-end">' . render_action_buttons($list_details["product_name"], "shared-data", $list_details["status"]) . '</td>
                                    </tr>';
                                        }
                                    }
                                    ?>
                ...
                <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                    <div
                        ...
                        <h6 class="fw-bold mb-0 text-primary">Shared Data Package Pricing (₦)</h6>
                        <button class="btn btn-sm btn-outline-primary rounded-pill px-3" type="button"
                            data-bs-toggle="collapse" data-bs-target="#bulkPriceCollapse">
                    ...
                    <div class="card-body p-4">
                        <div class="collapse mb-4" id="bulkPriceCollapse">
                            <div class="bg-light p-3 rounded-3 border">
                                <div class="row g-2 align-items-center">
                                    <div class="col-md-5"><input id="price-upgrade-input" type="number"
                                            placeholder="Value" class="form-control"></div>
                                    <div class="col-md-5">
                                    ...
                                    <div class="col-md-2"><button onclick="upgradeePriceDiscount();"
                                            class="btn btn-primary w-100">Apply</button></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <form method="post" action="">
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($item_name_array as $products) {
                                            ...
                                            if (empty($get_item_status_details) || empty($product_table))
                                                continue;
                                            ...
                                            if (mysqli_num_rows($product_smart_table) > 0) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end pe-3">
                                                        ...
                                                        '<a href="SharedData.php?action=disable&product_id=' . $product_smart_details["product_id"] . '&val_1=' . $product_smart_details["val_1"] . '&api_id=' . $product_smart_details["api_id"] . '" class="btn btn-outline-warning" title="Disable"><i class="bi bi-pause-fill"></i></a>' :
                                                        '<a href="SharedData.php?action=enable&product_id=' . $product_smart_details["product_id"] . '&val_1=' . $product_smart_details["val_1"] . '&api_id=' . $product_smart_details["api_id"] . '" class="btn btn-outline-success" title="Enable"><i class="bi bi-play-fill"></i></a>'
                                                    ) . '
                                                            <a href="SharedData.php?action=delete&product_id=' . $product_smart_details["product_id"] . '&val_1=' . $product_smart_details["val_1"] . '&api_id=' . $product_smart_details["api_id"] . '" class="btn btn-outline-danger" onclick="return confirm(\'Delete this package?\')" title="Delete"><i class="bi bi-trash"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>';
                                                    $csv_price_level_array[] = strtolower($products) . '_shared_data_' . str_replace(["_", "-"], "_", $product_smart_details["val_1"]) . "," . $product_smart_details["val_2"] . "," . $product_agent_details["val_2"] . "," . $product_api_details["val_2"] . "," . $product_api_details["val_3"];
                                                }
                                            }
                                        }
                                        ?>
                            ...
                            <div class="mt-4 pt-4 border-top">
                                <button name="update-price" type="submit"
                                    class="btn btn-primary btn-lg rounded-pill px-5 fw-bold shadow-sm">Save All
                                    Changes</button>
                            </div>
                        </form>
                    </div>
                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4">
                    <div
                        ...
                        <a href="javascript:void(0)"
                            onclick='downloadFile(`<?php echo implode("\n", $csv_price_level_array); ?>`, "shared-data.csv");'
                            class="btn btn-sm btn-light border"><i class="bi bi-download me-1"></i>Sample Template</a>
                    </div>
                    <div class="card-body p-4">
                        <form method="post" enctype="multipart/form-data">
                            <div class="input-group">
                                <input id="csv-chooser" type="file" class="form-control rounded-start-3" required />
                                <button data-no-lock onclick="getCSVDetails('5');" type="button"
                                    class="btn btn-primary px-4 fw-bold">Process CSV Upload</button>
                            </div>
                            <p class="small text-muted mt-2 mb-0"><i class="bi bi-info-circle me-1"></i>Ensure the
                                package names match exactly with the CSV header column.</p>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <?php if ($is_fetcher_allowed): ?>
        <script>
            ...
            async function fetchVariations() {
                ...
                try {
                    const response = await fetch(`ajax-fetch-plans.php?gateway=${encodeURIComponent(gateway)}&network=${encodeURIComponent(network)}&type=shared`);
            ...
            function applySinglePlan(index) {
                ...
                const network = document.getElementById('fetch-network').value;
                const inputId = `${network}_shared_data_${plan.code.toLowerCase().replace(/[\s\-]/g, '_')}_smart_level`;
