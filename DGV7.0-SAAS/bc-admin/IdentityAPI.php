<?php session_start();
    ...
    // configured on one page and sits behind the same premium activation.
      ...
      <div class="row g-4 justify-content-center">
        <div class="col-lg-10">
            ...
            <?php
                ...
                // every KYC integration the vendor can use - provider API, VoveID, NIN card, BVN verify -
                // is configured on this one page, and so VoveID sits behind the same premium activation as
                // the lookup providers. A vendor who had already configured VoveID keeps working
                // (grandfathered): a settings reorganisation must not switch off live verification.
                ...
                $voveid_key_saved = trim((string)($voveid_cfg['voveid_secret_key'] ?? '')) !== '' || trim((string)($voveid_cfg['voveid_public_key'] ?? '')) !== '';
                $voveid_ws_saved  = trim((string)($voveid_cfg['voveid_webhook_secret'] ?? ''));
                $voveid_unlocked  = !empty($identity_api_enabled) || $voveid_key_saved;
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                <?php if (!$voveid_unlocked): ?>
                    <div class="text-center py-5 bg-light rounded-4 border">
                        <i class="bi bi-lock-fill text-warning display-4 mb-3"></i>
                        <h4 class="fw-bold mb-2">VoveID is a premium identity provider</h4>
                        <p class="text-muted mb-4 px-lg-5">VoveID performs the entire check itself - ID document, selfie and liveness - which is what lets KYC be approved without a reviewer. Unlock the premium identity providers with the activation above, then configure VoveID here.</p>
                    </div>
                <?php else: ?>
                    <div class="alert alert-info border-0 rounded-4 small mb-4">
