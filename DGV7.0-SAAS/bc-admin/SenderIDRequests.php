    		...
    		if(in_array($status, $statusArray)){
    			...
    			if(mysqli_num_rows($select_sender_id) == 1){
    				...
    				if($status == 2){
                        ...
                        // Look up this vendor's PhilmoreSMS API key from sas_apis
                        $pms_api_q = mysqli_query($connection_server, "SELECT api_key FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' AND api_type='bulk-sms' AND api_base_url LIKE '%philmoresms.com%' AND status='1' LIMIT 1");
                        ...
                        $kudi_api_q = mysqli_query($connection_server, "SELECT api_key FROM sas_apis WHERE vendor_id='".$get_logged_admin_details["id"]."' AND api_type='bulk-sms' AND api_base_url LIKE '%kudisms.net%' AND status='1' LIMIT 1");
          ...
          <form method="post" action="">
            <div class="modal-body p-4">
                <div class="alert alert-warning border-0 small rounded-3 mb-3">
                    <i class="bi bi-info-circle me-1"></i>Saving will <strong>reset status to Pending</strong>. Re-approve to trigger PhilmoreSMS submission.
                </div>
                <div class="mb-3">
