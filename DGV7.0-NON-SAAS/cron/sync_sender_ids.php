...
echo "========================================================\n";
echo "Starting Sender ID Sync (NON-SAAS) - " . date('Y-m-d H:i:s') . "\n";
...
while ($row = mysqli_fetch_assoc($pending_q)) {
    ...
    // 2. Get the PhilmoreSMS API token for this vendor (NON-SAAS uses sas_api_gateway)
    $api_q = mysqli_query($connection_server, "SELECT api_key FROM sas_api_gateway WHERE vendor_id='$vendor_id' AND api_provider_name LIKE '%philmoresms.com%' LIMIT 1");
    ...
    if ($api_q && mysqli_num_rows($api_q) > 0) {
        ...
        echo "[SKIP] API not found in sas_api_gateway\n";
    }
}
