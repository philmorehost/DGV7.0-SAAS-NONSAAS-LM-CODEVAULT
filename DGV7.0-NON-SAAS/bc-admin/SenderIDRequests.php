    		...
    		if(in_array($status, $statusArray)){
    			...
    			if(mysqli_num_rows($select_sender_id) == 1){
    				...
    				if($status == 2){
                        ...
                        // Submit to PhilmoreSMS API if they are using it
                        $get_api = mysqli_query($connection_server, "SELECT * FROM sas_api_gateway WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_provider_name='philmoresms.com' LIMIT 1");
                        if(mysqli_num_rows($get_api) > 0) {
                            $api_detail = mysqli_fetch_array($get_api);
                            $api_token = trim($api_detail['api_key']);
                            $curl_url = "https://app.philmoresms.com/api/senderID.php";
