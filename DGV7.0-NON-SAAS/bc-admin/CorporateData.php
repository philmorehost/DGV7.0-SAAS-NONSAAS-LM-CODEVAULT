<?php session_start();
    ...
    if(isset($_POST["clear-all-plans"])){
        ...
        $api_q = mysqli_query($connection_server, "SELECT id FROM sas_apis WHERE vendor_id='$vid' AND api_type='cg-data'");
        $api_ids = [];
        ...
        if(!empty($api_ids)){
            ...
            $_SESSION["product_purchase_response"] = "All Corporate Data plans cleared successfully.";
        } else {
            ...
            $_SESSION["product_purchase_response"] = "Corporate Data plans cleared from pricing table.";
        }
        header("Location: CorporateData.php");
        exit();
    }
    ...
    if(isset($_POST["install-product"])){
        ...
        $product_varieties = array(
            "mtn" => array("500mb","1gb","2gb","3gb"),
            "airtel" => array(),
            "glo" => array("500mb", "1gb", "2gb", "3gb", "5gb", "10gb"),
            "9mobile" => array("500mb","1gb","2gb","3gb","5gb","10gb")
        );
        $account_level_table_name_arrays = array("sas_smart_parameter_values", "sas_agent_parameter_values", "sas_api_parameter_values");
        install_product($connection_server, $get_logged_admin_details, "cg-data", "sas_cg_data_status", $products_array, $product_varieties, $account_level_table_name_arrays);
    }
...
<body>
    ...
    <div class="pagetitle">
      <h1>CORPORATE DATA API</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="#">Home</a></li>
          <li class="breadcrumb-item active">Corporate Data</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section dashboard">
      <div class="row g-4">
        <?php
            ...
            $current_api_type = 'cg-data';
            // Only these gateways actually support live plan/price fetching -- every other configured
            // API has no fetch endpoint, so it's deliberately excluded from this dropdown rather than
            // listed and left to fail when selected.
            $installed_gateways = [];
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0"><i class="bi bi-cloud-arrow-down me-2 text-primary"></i>Corporate/Gifting Data Package Plan Variation Fetcher</h5>
                    ...
                    <div class="collapse mb-3" id="fetcherGuide">
                        <div class="alert alert-info border-0 shadow-sm rounded-4 mb-0">
                            ...
                            <ol class="small mb-0 text-dark">
                                ...
                                <li>Click <strong>Fetch Plans</strong> to load the latest available Corporate/Gifting packages from the provider.</li>
        ...
        <div class="col-lg-4">
          <div class="card shadow-sm border-0 rounded-4 mb-4">
            ...
            <div class="card-body p-4 text-center">
                <p class="small text-muted mb-4">Wipe all Corporate Data plans from the pricing table. Use this when switching API providers.</p>
                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL Corporate Data pricing plans? This cannot be undone.');">
                    <button name="clear-all-plans" type="submit" class="btn btn-outline-danger w-100 rounded-3 fw-bold">
                        <i class="bi bi-eraser-fill me-2"></i>Clear All Corp Plans
                    </button>
                </form>
            </div>
          </div>
        ...
        <div class="col-lg-8">
                            ...
                            if(mysqli_num_rows($select_item_lists) >= 1){
                                while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                    ...
                                    <tr>
                                        ...
                                        <td>'.$status_badge.'</td>
                                        <td class="pe-4 text-end">'.render_action_buttons($list_details["product_name"], "cg-data", $list_details["status"]).'</td>
                                    </tr>';
                                }
                            }
                        ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end pe-3">
                                                        <div class="btn-group btn-group-sm">
                                                            ...
                                                            '<a href="CorporateData.php?action=toggle_status&api_id='.$product_smart_details["api_id"].'&product_id='.$product_smart_details["product_id"].'&val_1='.urlencode($product_smart_details["val_1"]).'&current_status=1" class="btn btn-outline-warning" title="Disable"><i class="bi bi-pause-fill"></i></a>' :
                                                            '<a href="CorporateData.php?action=toggle_status&api_id='.$product_smart_details["api_id"].'&product_id='.$product_smart_details["product_id"].'&val_1='.urlencode($product_smart_details["val_1"]).'&current_status=0" class="btn btn-outline-success" title="Enable"><i class="bi bi-play-fill"></i></a>'
                                                            ).'
                                                        </div>
                                                    </td>
                                                </tr>';
                                                $csv_price_level_array[] = strtolower($products).'_corporate_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).",".$product_smart_details["val_2"].",".$product_agent_details["val_2"].",".$product_api_details["val_2"].",".$product_api_details["val_3"];
                                            }
                                        }
                                    }
                                ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="javascript:void(0)" onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "corporate-data.csv");' class="btn btn-sm btn-light border"><i class="bi bi-download me-1"></i>Sample Template</a>
                </div>
                <div class="card-body p-4">
    ...
    async function fetchVariations() {
        ...
        try {
            const response = await fetch(`ajax-fetch-plans.php?gateway=${encodeURIComponent(gateway)}&network=${encodeURIComponent(network)}&type=cg`);
    ...
    function applySinglePlan(index) {
        ...
        const network = document.getElementById('fetch-network').value;
        const inputId = `${network}_corporate_data_${plan.code.toLowerCase().replace(/[\s\-]/g, '_')}_smart_level`;
