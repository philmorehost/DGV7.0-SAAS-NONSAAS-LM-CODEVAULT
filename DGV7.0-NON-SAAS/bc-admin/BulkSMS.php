<?php session_start();
    ...
    if(isset($_POST["clear-all-plans"])){
        ...
        $api_q = mysqli_query($connection_server, "SELECT id FROM sas_apis WHERE vendor_id='$vid' AND api_type='bulk-sms'");
        $api_ids = [];
        ...
        if(!empty($api_ids)){
            ...
            $_SESSION["product_purchase_response"] = "All Bulk SMS plans cleared successfully.";
        } else {
            $_SESSION["product_purchase_response"] = "No Bulk SMS API configured to clear plans for.";
        }
        header("Location: BulkSMS.php");
        exit();
    }

    if(isset($_GET["action"]) && isset($_GET["product_id"]) && isset($_GET["val_1"])){
        ...
        if($action == "enable" || $action == "disable"){
            ...
            foreach($pricing_tables as $table){
                mysqli_query($connection_server, "UPDATE $table SET status='$new_status' WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_id='$product_id' && val_1='$val_1'");
            }
            $_SESSION["product_purchase_response"] = "Package status updated successfully.";
    
        } elseif($action == "delete"){
            foreach($pricing_tables as $table){
                mysqli_query($connection_server, "DELETE FROM $table WHERE vendor_id='".$get_logged_admin_details["id"]."' && product_id='$product_id' && val_1='$val_1'");
            }
    ...
    if(isset($_POST["install-product"])){
        ...
        $product_varieties = array(
            "mtn" => array("standard_sms"),
            "airtel" => array("standard_sms"),
            "glo" => array("standard_sms"),
            "9mobile" => array("standard_sms")
        );
        $account_level_table_name_arrays = array("sas_smart_parameter_values", "sas_agent_parameter_values", "sas_api_parameter_values");
        install_product($connection_server, $get_logged_admin_details, "bulk-sms", "sas_bulk_sms_status", $products_array, $product_varieties, $account_level_table_name_arrays);
    }
...
<body>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">BulkSMS</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0"><i class="bi bi-cloud-arrow-down me-2 text-primary"></i>Bulk SMS Plan Variation Fetcher</h5>
                    ...
                    <div class="collapse mb-3" id="fetcherGuide">
                        <div class="alert alert-info border-0 shadow-sm rounded-4 mb-0">
                            ...
                            <ol class="small mb-0 text-dark">
                                ...
                                <li>Click <strong>Fetch Plans</strong> to load the latest SMS pricing from the provider.</li>
                                <li>Click <strong>Apply</strong> on a plan to update the pricing table below, or click <strong>Apply All</strong> to save directly to your database.</li>
        ...
        <div class="col-lg-4">
          <div class="card shadow-sm border-0 rounded-4 mb-4">
            ...
            <div class="card-body p-4 text-center">
                <p class="small text-muted mb-4">Wipe all Bulk SMS plans from the pricing table. Use this when switching API providers.</p>
                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL Bulk SMS pricing plans? This cannot be undone.');">
                    <button name="clear-all-plans" type="submit" class="btn btn-outline-danger w-100 rounded-3 fw-bold">
                        <i class="bi bi-eraser-fill me-2"></i>Clear All SMS Plans
                    </button>
                </form>
            </div>
          </div>
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom">
                    <h6 class="fw-bold mb-0 text-primary">Installed Bulk SMS Status</h6>
                </div>
                            ...
                            if(mysqli_num_rows($select_item_lists) >= 1){
                                while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                    ...
                                    <tr>
                                        ...
                                        <td>'.$status_badge.'</td>
                                        <td class="pe-4 text-end">'.render_action_buttons($list_details["product_name"], "bulk-sms", $list_details["status"]).'</td>
                                    </tr>';
                                }
                            }
                        ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-primary">Bulk SMS Pricing (Per Unit)</h6>
                ...
                <div class="card-body p-4">
                    ...
                    <form method="post" action="">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light">
                            <tr>
                                <th>Network</th><th>Smart (₦)</th><th>Agent (₦)</th><th>API (₦)</th><th>Status</th><th class="text-end pe-3">Actions</th>
                            </tr>
                            </thead>
                            <tbody>
                                <?php
                                    foreach($item_name_array as $products){
                                        ...
                                        $product_smart_table = mysqli_query($connection_server, "SELECT * FROM sas_smart_parameter_values WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_id='".($get_item_status_details["api_id"] ?? "")."' && product_id='".($product_table["id"] ?? "")."' AND val_1 = 'standard_sms'");
                                        $product_agent_table = mysqli_query($connection_server, "SELECT * FROM sas_agent_parameter_values WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_id='".($get_item_status_details["api_id"] ?? "")."' && product_id='".($product_table["id"] ?? "")."' AND val_1 = 'standard_sms'");
                                        $product_api_table = mysqli_query($connection_server, "SELECT * FROM sas_api_parameter_values WHERE vendor_id='".$get_logged_admin_details["id"]."' && api_id='".($get_item_status_details["api_id"] ?? "")."' && product_id='".($product_table["id"] ?? "")."' AND val_1 = 'standard_sms'");
                                        ...
                                        if(mysqli_num_rows($product_smart_table) > 0){
                                            while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                                ...
                                                if ($product_smart_details['status'] == 1) {
                                                    $actions .= '<a href="BulkSMS.php?action=disable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-sm btn-light border text-warning"><i class="bi bi-power"></i></a> ';
                                                } else {
                                                    $actions .= '<a href="BulkSMS.php?action=enable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-sm btn-light border text-primary"><i class="bi bi-power"></i></a> ';
                                                }
                                                $actions .= '<a href="BulkSMS.php?action=delete&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'" class="btn btn-sm btn-light border text-danger" onclick="return confirm(\'Delete this package?\')"><i class="bi bi-trash"></i></a>';
                                                ...
                                                $csv_price_level_array[] = strtolower($products).",".$product_smart_details["val_2"].",".$product_agent_details["val_2"].",".$product_api_details["val_2"];
                                            }
                                        }
                                    }
                                ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="javascript:void(0)" onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "bulk-sms.csv");' class="btn btn-sm btn-light border"><i class="bi bi-download me-1"></i>Sample Template</a>
                </div>
                <div class="card-body p-4">
    ...
    async function fetchVariations() {
        ...
        try {
            const response = await fetch(`ajax-fetch-plans.php?gateway=${encodeURIComponent(gateway)}&network=${encodeURIComponent(network)}&type=bulk-sms`);
            ...
            fetchedPlans.forEach((plan, index) => {
                ...
                const row = `
                    <tr>
                        ...
                        <td>₦${providerPrice.toFixed(4)}</td>
                        <td class="text-primary fw-bold">₦${plan.smartPrice.toFixed(4)}</td>
                        <td class="text-success fw-bold">₦${profit.toFixed(4)}</td>
                        <td>
    ...
    function applySinglePlan(index) {
        ...
        if(input) {
            input.value = plan.smartPrice.toFixed(4);
            ...
            if(agentInput) agentInput.value = plan.agentPrice.toFixed(4);
            if(apiInput) apiInput.value = plan.apiPrice.toFixed(4);
            ...
            alert(`Applied ${plan.name} pricing to table. Remember to click Save All Changes.`);
        } else {
