<?php session_start();
    ...
    if(!isset($_SESSION["temp_admin_session"]) || !isset($_SESSION["auth_otp_time"])){
        header("Location: Login.php");
        exit();
    }

    $email = $_SESSION["temp_admin_session"];
    $vendor_id = resolveVendorID();
    $admin_detail = mysqli_fetch_assoc(mysqli_query($connection_server, "SELECT * FROM sas_vendors WHERE id='$vendor_id' AND email='$email' LIMIT 1"));
    ...
    if(isset($_POST["verify-otp"])){
        ...
        if($_SESSION["2fa_type"] == "email"){
            if (time() - $_SESSION["auth_otp_time"] > 600) {
                ...
                unset($_SESSION["temp_admin_session"]);
    ...
    if(isset($_GET["resend"])){
        if($_SESSION["2fa_type"] == "email"){
            ...
            $body = "Your admin verification code is: <b>$otp</b>";
            sendVendorEmail($admin_detail["email"], $subject, $body);
...
<!DOCTYPE html>
<head>
    <title>Verify Admin Login | <?php echo $get_all_super_admin_site_details["site_title"]; ?></title>
    <meta charset="UTF-8" />
        ...
        <div class="card shadow border-0 rounded-4 p-4 p-md-5" style="max-width: 450px; width: 100%;">
            <div class="text-center mb-4">
                <div class="bg-primary bg-opacity-10 p-3 rounded-circle d-inline-block mb-3">
                    <i class="bi bi-shield-lock-fill text-dark-primary fs-1"></i>
                </div>
                <h3 class="fw-bold">Admin Verification</h3>
                <p class="text-muted">Enter the 2FA code sent to your admin email: <b><?php echo substr($admin_detail['email'], 0, 3) . '...' . substr($admin_detail['email'], strpos($admin_detail['email'], '@')); ?></b></p>
            </div>
            ...
            <form method="post" action="">
                <div class="mb-4">
                    <label class="form-label small fw-bold text-uppercase text-muted text-center d-block">6-Digit Admin Code</label>
                ...
                <div class="text-center">
                    <a href="?resend=1" class="text-decoration-none small fw-bold">Resend Code</a>
                </div>
            </form>
        </div>
    </div>
