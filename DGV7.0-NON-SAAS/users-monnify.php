<?php session_start();
    ...
    if (!$select_vendor_table) {
        // Try to identify via product reference (accountReference) if host doesn't match
        ...
        // Fallback for checkout transactions (Fund wallet / Submit payment)
	...
	if($select_vendor_table && $select_vendor_table["status"] == 1){
            ...
            if($pay_status == "PAID") {
                ...
                if (!empty($username)) {
                    ...
                    if (mysqli_num_rows($check_tx) == 0) {
                        ...
                        chargeOtherUser($username, "credit", "Monnify", "Wallet Credit", $new_ref, $transaction_ref, $amount_paid, $amount_deposited, $desc, "WEB", $host, 1);
                        echo "SUCCESS";
                    } else {
        ...
        error_log("Monnify Webhook: Vendor not found or inactive for host $host");
    }
