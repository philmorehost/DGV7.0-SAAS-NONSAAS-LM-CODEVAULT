        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($row = mysqli_fetch_assoc($result)) {
                    ...
                    // Standalone/Development fallback: Use vendor with ID 1 if domain doesn't match
                    $q_fallback = mysqli_query($connection_server, "SELECT * FROM sas_vendors WHERE id = 1 LIMIT 1");
                    if ($q_fallback && mysqli_num_rows($q_fallback) > 0) {
                        $vendor_account_details = mysqli_fetch_assoc($q_fallback);
                    } else {
