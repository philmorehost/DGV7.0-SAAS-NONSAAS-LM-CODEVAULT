...
fwrite(fopen("bank.txt", "a"), $incoming_post_request . "\n\n");
...
if (json_last_error() === JSON_ERROR_NONE) {
    ...
    if ($select_vendor_table) {
        ...
        $bank_code = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["bank_code"])));
        $account_number = str_replace(" ", "", mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["account_number"]))));
        $amount = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["amount"])));
        $amount = str_replace(["-", "+", "*", "/"], "", $amount);
        $narration = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["narration"])));

        $status_update = "failed";
        $status_msg = "Unknown Error";
        $bank_owner_name = "Unknown User";
        $transfer_enquiry_id = "";
        ...
        if (
            ...
            is_numeric($account_number) &&
            (strlen($account_number) == 10) &&
            !empty($amount) &&
            is_numeric($amount) &&
            ($amount >= 100) &&
            !empty($narration)
            ...
            if (mysqli_num_rows($checkuser) == 1) {
                ...
                if (mysqli_num_rows($checkuser_pass) == 1) {
                    ...
                    $api_file_url = $_SERVER["DOCUMENT_ROOT"] . "/web/api/verify-bank.php";
                    if (file_exists($api_file_url)) {
                        $api_post_info_from_app = array("api_key" => $get_user_info["api_key"], "bank_code" => $bank_code, "account_number" => $account_number, "amount" => $amount, "narration" => $narration);
                        ...
                        if (in_array($json_response_array["status"], array("success", "pending"))) {
                            ...
                            $bank_owner_name = $json_response_array["customer_name"];
                            $transfer_enquiry_id = $json_response_array["enquiry_id"];
                        } else {
                            $status_msg = $json_response_array["desc"];
                        }
                    } else {
            ...
            if (empty($username)) {
                ...
                $status_msg = "Account number is required";
            } elseif (!is_numeric($account_number)) {
                $status_msg = "Account number must be a number";
            } elseif ((strlen($account_number) < 10) || (strlen($account_number) > 10)) {
                $status_msg = "Account number must be 10 digits long";
            } elseif (empty($amount)) {
                ...
                $status_msg = "Narration is required";
            }
        }
        $app_json = json_encode(array("json-status" => "success", "status" => $status_update, "status-msg" => $status_msg, "name" => $bank_owner_name, "enquiry_id" => $transfer_enquiry_id), true);
    } else {
...
fwrite(fopen("bank.json", "a"), $app_json . "\n\n");
