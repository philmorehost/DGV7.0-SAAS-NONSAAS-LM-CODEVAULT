...
if (json_last_error() === JSON_ERROR_NONE) {
    ...
    if ($select_vendor_table) {
        ...
        $enquiry_id = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["enquiry_id"])));
        ...
        if (
            ...
            if (mysqli_num_rows($checkuser) == 1) {
                ...
                if (mysqli_num_rows($checkuser_pass) == 1) {
                    ...
                    $api_file_url = $_SERVER["DOCUMENT_ROOT"] . "/web/api/bank-transfer.php";
                    if (file_exists($api_file_url)) {
                        $api_post_info_from_app = array("api_key" => $get_user_info["api_key"], "bank_code" => $bank_code, "account_number" => $account_number, "enquiry_id" => $enquiry_id, "amount" => $amount, "narration" => $narration);
            ...
            if (empty($username)) {
                ...
                $status_msg = "Account type is required";
            } elseif (empty($amount)) {
