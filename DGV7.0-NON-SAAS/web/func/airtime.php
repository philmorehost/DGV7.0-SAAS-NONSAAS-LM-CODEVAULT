...
if (in_array($purchase_method, $purchase_method_array)) {
    ...
    $status = 3;
    // Transaction PIN enforcement - only for a PERSON buying: our own website ("WEB") or the mobile
    // app ("APP"). External API integrations ("API") are deliberately EXEMPT: one merchant api_key
    // serves that merchant's own customers, so there is no single PIN to ask for, and requiring one
    // made every API purchase fail with "Invalid transaction PIN.".
                                ...
                                if ($api_detail["status"] == 1) {
                                    ...
                                    if (($product_table["status"] == 1) && ($product_status_table["status"] == 1)) {
                                        if (productIDBlockChecker($phone_no) == "success") {
                                            if (productIDPurchaseChecker($phone_no, "airtime", $purchase_method) == "success") {
