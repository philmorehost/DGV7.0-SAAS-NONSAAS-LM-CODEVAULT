    ...
    if (mysqli_num_rows($get_user_query) == 1) {
        if (($get_logged_user_details["api_status"] == 1 || $purchase_method == "app") && $get_logged_user_details["status"] == 1) {
            ...
            foreach ($exams as $exam) {
                ...
                $status_query = mysqli_query($connection_server, "SELECT * FROM sas_exam_status WHERE vendor_id='" . $get_logged_user_details["vendor_id"] . "' && product_name='$exam'");
                $status = mysqli_fetch_array($status_query);

                if ($status && !empty($status['api_id'])) {
                    $api_id = $status['api_id'];

                    $product_query = mysqli_query($connection_server, "SELECT * FROM sas_products WHERE vendor_id='".$get_logged_user_details["vendor_id"]."' && product_name='$exam' LIMIT 1");
