...
class CableFragment : Fragment(R.layout.fragment_cable) {
    ...
    /** Maps provider key Ã¢â ’ container LinearLayout */
    ...
    /** Maps provider key Ã¢â ’ logo ImageView (for greying out unselected providers) */
    ...
    private fun updatePlanDropdown() {
        ...
        val hint = if (plans.isEmpty()) "No plans for ${selectedProvider.uppercase()} Ã¢â¬” contact support" else "Select Package"
    ...
    private fun confirmPayment() {
        ...
        if (verifiedSmartcard != smartcard) {
            snack("SmartCard number changed Ã¢â¬” please verify again"); return
        }
