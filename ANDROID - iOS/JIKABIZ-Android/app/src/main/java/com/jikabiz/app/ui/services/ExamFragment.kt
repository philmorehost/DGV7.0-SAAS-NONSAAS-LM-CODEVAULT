...
import com.jikabiz.app.databinding.FragmentExamBinding
...
class ExamFragment : Fragment(R.layout.fragment_exam) {
    ...
    private fun updateProductDropdown() {
        ...
        binding.spinnerProduct.setOnItemClickListener { _, _, pos, _ ->
            products.getOrNull(pos)?.let { p ->
                ...
                binding.tvPriceInfo.text = "Price: ₦$selectedProductPrice per PIN"
            }
        }
    }
    ...
    private fun doPurchase() {
        ...
        lifecycleScope.launch {
            try {
                ...
                //   "type"     → exam body (waec/neco/nabteb/jamb)
                //   "quantity" → product/pin type code (val_1 in price table, e.g. "result_checker")
    ...
    private fun showResult(ref: String, details: String) {
        ...
        MaterialAlertDialogBuilder(requireContext())
            .setTitle("✅ ${selectedExam.uppercase()} PIN Purchased")
