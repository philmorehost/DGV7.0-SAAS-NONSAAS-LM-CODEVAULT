package com.jikabiz.app.ui.services
...
import com.jikabiz.app.databinding.FragmentGiftcardBinding
import com.jikabiz.app.util.LoadingOverlay
...
class GiftCardFragment : Fragment(R.layout.fragment_giftcard) {
    ...
    private fun onProductSelected(product: Map<String, Any>) {
        ...
        if (type == "RANGE") {
            ...
            binding.tvRangeHint.text = "Min: $selectedProductCurrency ${minVal.toInt()} — Max: $selectedProductCurrency ${maxVal.toInt()}"
    ...
    private fun updatePricePreview() {
        if (selectedAmount > 0 && selectedProductId > 0) {
            binding.tvPricePreview.text = "≈ $selectedProductCurrency ${selectedAmount.toInt()} (Rate applies at checkout)"
    ...
    private fun buildCardView(card: Map<String, Any>): View {
        ...
        inner.addView(textView("$name — $currency ${value.toInt()}", 16f, bold = true))
        ...
        if (isForSale == 1) {
            inner.addView(textView("Listed for ₦${salePrice.toLong()}", 13f, colorRes = R.color.success))
        } else if (cardId > 0) {
    ...
    private fun buildListingView(listing: Map<String, Any>): View {
        ...
        inner.addView(textView("$product — $currency ${value.toInt()}", 16f, bold = true))
        inner.addView(textView("Seller: @$seller", 13f, colorRes = R.color.text_secondary))
        inner.addView(textView("Price: ₦${priceNgn.toLong()}", 14f, colorRes = R.color.primary))
    ...
    private fun showP2PPinDialog(listingId: Int, productName: String, priceNgn: Double) {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setMessage("$productName — ₦${priceNgn.toLong()}")
            .setView(pinView)
            .setPositiveButton("Confirm") { _, _ ->
                val pin = etPin?.text?.toString()?.trim() ?: ""
